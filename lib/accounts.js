/* ------------------------------------------------------------------
   ...
   screens alike. The rule is the same one lib/waitlist.js follows: the
...
/* Every text input is capped server-side. Same shape as
   lib/waitlist.js FIELD_LIMITS so the two read alike. */
export const LIMITS = {
  name: 120,
  email: 160,
  handle: 32,
...
/* Blob layout for profile, company and team images:
     avatars/<user-id>/<file>
     company-logos/<user-id>/<file>
     team-logos/<team-id>/<file>
   parseUploadPath() is the security boundary — the token route
   refuses to sign anything it rejects.

   A company logo is owned by the person who uploaded it, not by an
   organisation row: a prospective sponsor has an account long before
   anyone records a sponsorship, and the logo is the thing we ask for
   at onboarding. So it scopes by user id, exactly like an avatar. */
export const UPLOAD_KINDS = ["avatar", "company-logo", "team-logo"];
...
/* The logo field is asked for in two places — onboarding and the
   dashboard profile — so it's written once. A sponsor should read the
   same sentence whether they upload it the day they sign up or three
   weeks later. */
export const COMPANY_LOGO_COPY = {
  label: "Your logo",
  hint: "It ends up on the site, the sponsor wall and the slides. Send the version that reads on black — this place is dark, and a mark with a white box baked in shows the box.",
  fine: "Transparent PNG if you have one. Up to 5 MB.",
};
