/* ------------------------------------------------------------------
   Day One — the beginner-to-intermediate series, four sessions.

   Same shape as WORKSHOPS in hackathon.js so every program surface can
   read either one. Dates are not set yet: `date` and `iso` are null on
   purpose and everything downstream keys off `iso == null` to render
   "Dates TBD" — no RSVP row, no Event JSON-LD, no relative timeframes.
   When the dates land, fill both fields in here and the schema, the
   schedules and the copy turn on with no route changes.

   No luma/meetup/linkedin fields while TBD; announcements wait for
   real dates.
...
export const DAY_ONE_STEPS = [
  {
    name: "Use",
    copy: "Threads that end in an artifact you keep, and skills so you only have to explain yourself once.",
    range: "Session 01",
  },
  {
    name: "Co-work",
...
export const DAY_ONE_SESSIONS = [
  {
    ...
    act: "Use",
    copy: "Most people use these tools like a search box, and get search-box answers back. This is the other way to work: threads that end in an artifact you keep, and skills so you explain yourself once instead of every time. Built live on one real business — the one you carry through all four nights.",
    take: "A brand voice skill and an ICP skill built from your own material, the positioning one-pager they produce, and your first article drafted.",
    lede:
      "Session one is where you pick the business. Everything across the next three nights gets built on it, so the first ten minutes are spent naming it out loud. Then the real work: what separates a thread that produces something from a thread that produces conversation, and how to turn the thing you keep re-explaining into a skill file you run with one word. Follow along on a laptop, or just watch.",
    why:
  ...
  },
  {
    ...
    skills: ["content-calendar", "weekly-report", "icp-builder", "outbound-sequence"],
  },
  {
    n: "03",
    slug: "first-deploy",
  ...
  },
  {
    ...
    skills: ["support-agent", "scheduled-workflow"],
  },
