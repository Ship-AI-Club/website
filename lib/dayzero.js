...
export const DAY_ZERO_SESSIONS = [
  {
    ...
    title: "Skills, agents, and a sneak peek at Grok Bot",
    act: "Use",
    copy: "Most people meet AI as a chat box, then get told they need a framework and a cloud deploy before an agent can help. They don't. A skill is the thing you keep re-explaining, written once as a markdown file the agent reads by itself. Tonight we write one live, run it with one word, and close on Grok Bot answering — same pieces.",
    take: "A skill file written from something you already repeat, tested in a fresh thread with one word, and edited once from what the first run got wrong.",
    lede:
    ...
    agenda: [
      { t: "The long way around", c: "Google's Agent Development Kit — a real way to build agents, and a lot of machinery before the first useful sentence. The foil for everything after it." },
      { t: "What a skill is", c: "Instructions written once, read automatically every time. The thing you keep re-explaining, turned into a file." },
      { t: "What a markdown file is", c: "A plain text file with headings, lists, and bold. A skill is that file plus two lines of frontmatter. No framework, no deploy." },
      { t: "Start a thread that works", c: "Context, role, task, format, and the artifact you keep. We start one live and watch the difference from a conversation." },
      { t: "Write the skill", c: "Take what you just explained, save it as a skill file, run it in a fresh thread with one word." },
      { t: "Iterate", c: "A skill is never right the first time. Run it, read what sounded wrong, edit the file, run again." },
      { t: "A sneak peek at Grok Bot", c: "A live demo. Same pieces — skills, markdown, an agent — answering in front of the room." },
    ],
    bring: "A laptop if you want to follow along. A free ChatGPT or Claude account is enough for tonight. Following along is optional; plenty of people just watch the first one.",
    commit: "01-skills/ — if you want somewhere to put it: the skill you wrote tonight, and a line on what the first run got wrong.",
    skills: ["skill-writer"],
  },
  {
    n: "02",
