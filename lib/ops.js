...
export async function checkinRoster(day = eventDay()) {
  ...
  return safeRead(
    () => sql`
      select u.id, u.name, u.handle, u.email, u.avatar_url, r.dietary,
...
export async function cateringSummary() {
  const [registeredRows, checkinRows, dietaryRows] = await Promise.all([
    safeRead(
      () => sql`select count(*)::int as registered from registrations where withdrawn_at is null`,
      [{ registered: 0 }],
    ),
    safeRead(
      () => sql`
        select c.day, count(distinct c.user_id)::int as n
          ...
          join registrations r on r.user_id = c.user_id and r.withdrawn_at is null
    ...
    safeRead(
      () => sql`
        select min(trim(r.dietary)) as dietary, count(*)::int as n
          from registrations r
         where r.withdrawn_at is null and trim(r.dietary) <> ''
         group by lower(trim(r.dietary))
         order by lower(trim(r.dietary))`,
      [],
    ),
  ]);
...
export async function exportRows(type) {
  if (type === "attendees") {
    return safeRead(
      () => sql`
        select u.id, u.name, u.handle, u.email, r.track, r.dietary, r.registered_at
  ...
  if (type === "dietary") {
    return safeRead(
      () => sql`
        select u.id, u.name, u.handle, u.email, r.dietary, r.registered_at
         from registrations r join users u on u.id = r.user_id
         where r.withdrawn_at is null and trim(r.dietary) <> ''
         order by lower(trim(r.dietary)), coalesce(nullif(u.name, ''), u.email)`,
      [],
    );
  }
  if (type === "checkins") {
