...
import { EVENT, WORKSHOPS, ACTS, TEMPLATE_REPO, venueOf } from "./hackathon.js";
...
export const PROGRAMS = [
  ...
  },
  {
    ...
    description:
      "Four free beginner-to-intermediate AI workshops at Workuity in Phoenix. Session one is Tuesday Aug 25 at Workuity Biltmore and Wednesday Aug 26 at Workuity Chandler, 6:00 PM — same session, pick the room. Remaining nights announced soon. Get real work out of ChatGPT and Claude, put routines and reports on a schedule, ship a marketing site with programmatic SEO, then add a support agent and a workflow that runs while you sleep.",
    datesLabel: "Aug 25–26, then TBD",
    datesCopy:
      "Session one is Tuesday Aug 25 at Workuity Biltmore and Wednesday Aug 26 at Workuity Chandler, evenings at 6:00 PM. Same session — pick the room that works. Nights two through four announced soon; watch Discord and the Luma and Meetup calendars.",
    startISO: null,
    endISO: null,
    defaultVenue: "workuity",
    venueNote:
      "Session one runs twice: Tuesday Aug 25 at Workuity Biltmore, Wednesday Aug 26 at Workuity Chandler. Same session — pick the room. Workuity is the venue sponsor.",
...
export function sessionNights(w) {
  if (Array.isArray(w?.nights) && w.nights.length > 0) return w.nights;
  if (w?.iso) return [{ date: w.date, iso: w.iso, venue: w.venue }];
  return [];
}

export function sessionScheduled(w) {
  return sessionNights(w).length > 0;
}

export function sessionDateLabel(w) {
  const nights = sessionNights(w);
  if (nights.length === 0) return "Dates TBD";
  const year = nights[0].iso.slice(0, 4);
  if (nights.length === 1) return `${nights[0].date}, ${year}`;
  return `${nights.map((night) => night.date).join(" & ")}, ${year}`;
}

export function sessionVenues(w, program) {
  const nights = sessionNights(w);
  const keys = nights.length
    ? nights.map((night) => night.venue || w.venue || program?.defaultVenue)
    : [w?.venue || program?.defaultVenue];
  return [...new Set(keys.filter(Boolean))].map((key) => venueOf({ venue: key }));
}

export function programVenueNames(program) {
  return [
    ...new Set(
      program.sessions.flatMap((w) => sessionVenues(w, program).map((venue) => venue.name))
    ),
  ];
}
