...
export async function leaderboard() {
  ...
  const rows = [...byId.values()].map((s) => ({
    ...
    perAxis: Object.fromEntries(
      RUBRIC.map((c) => {
        const values = s.cards.map((card) => card[c.key]).filter((v) => v !== null);
...
/** Public read — used by /hackathon/certificate/<id>, so it degrades. */
...
/** Public read — the entrant listing on /hackathon/results. */
